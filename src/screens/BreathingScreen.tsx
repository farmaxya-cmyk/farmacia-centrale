...
const BreathingScreen = () => {
    ...
    // FIX: Percorsi relativi (senza slash iniziale) per funzionare in sottocartelle (es. GitHub Pages)
    const MUSIC_TRACKS = [
    ...
    const playCue = React.useCallback(() => {
        if (mode === 'closed' && cueRef.current) {
            ...
            // VOLUME ALZATO AL MASSIMO per contrastare la musica di sottofondo
            cueRef.current.volume = 1.0; 
            
            const playPromise = cueRef.current.play();
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    console.log("Cue sound play prevented:", error);
                });
            }
        }
    }, [mode]);
            ...
            if (audioRef.current) {
                ...
                if (playPromise !== undefined) {
                    playPromise.catch(error => {
                        ...
                        alert("Impossibile riprodurre l'audio. Verifica i permessi del browser.");
                    });
                }
            }
        }
        ...
        <div className="p-4 md:p-8 animate-fade-in flex flex-col items-center justify-center min-h-full">
            ...
            <audio 
                ...
                onError={(e) => {
                    ...
                    // alert(`Errore: Impossibile caricare il file audio: ${musicTrack}`);
                }}
            />
            {/* Cue Sound Player */}
            <audio 
