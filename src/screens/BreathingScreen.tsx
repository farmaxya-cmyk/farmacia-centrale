...
const BreathingScreen = () => {
    ...
    // NOTA: Assicurati che i file esistano in public/audio/ con questi nomi esatti (case-sensitive)
    const MUSIC_TRACKS = [
    ...
    const playCue = React.useCallback(() => {
        if (mode === 'closed' && cueRef.current) {
            ...
            // VOLUME ABBASATO A 0.1 (10%)
            cueRef.current.volume = 0.1; 
            ...
            if (audioRef.current) {
                ...
                if (playPromise !== undefined) {
                    playPromise.catch(error => {
                        ...
                        alert("Impossibile riprodurre l'audio. Verifica che il file esista nella cartella 'public/audio'.");
                    });
                }
            }
        }
    ...
    return (
        <div className="p-4 md:p-8 animate-fade-in flex flex-col items-center justify-center min-h-full">
            ...
            {/* Background Music Player - KEY prop is crucial here for track switching */}
            <audio 
                key={musicTrack} 
                ref={audioRef} 
                src={musicTrack}
                loop
                preload="auto"
                onError={(e) => console.error("Errore caricamento audio (verificare percorso file):", e)}
            />
